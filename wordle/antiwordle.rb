...
@used_chars = Set.new(ARGV[1].to_s.chars)
@words = CSV.read("words.csv", headers: true).map do |row|
  ...
  green = row["green"].split(" ").map(&:to_i)
  {
    ...
    green: green,
    yellow: yellow,
    score: yellow.values.sum + green.sum / 2,
  }
...
def find_best_word(without_chars, start_at)
  best = @words[start_at..-1].find.with_index do |w, i|
    next if w[:word] == "susus" || w[:word] == "rurus" || w[:word] == "sours"
    w[:chars].disjoint?(without_chars | @used_chars) &&
      w[:word][2] == "u" &&
      w[:word][3] == "r" &&
      w[:word][4] == "s"
  end
